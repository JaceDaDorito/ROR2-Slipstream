...
namespace Slipstream.Items.Yellow
{
    public static class RottingPerforator
    {
        //public static ItemDef itemDef = ScriptableObject.CreateInstance<ItemDef>();

        public static void Init() //This runs whatever methods you need to for the item, such as language stuff, effects, setting up the item, etc. Init is ran in the Awake() method in the Main.cs file and every item you add you have to run Init() inside of Awake() in the Main.cs file or else it won't work.
        {
        ...
        private static void RottingPerforatorItem()
        {
        ...
        private static void Hooks()
        {
            ...
            On.RoR2.GlobalEventManager.OnCharacterDeath += (On.RoR2.GlobalEventManager.orig_OnCharacterDeath orig, GlobalEventManager self, DamageReport damageReport) =>
            {
                if (damageReport?.attackerBody && damageReport?.attackerMaster)
                {
                    ...
                    if (character.inventory.GetItemCount(itemDef) > 0 && Util.CheckRoll(10f, damageReport.attackerMaster) && damageReport.victimBody.inventory.GetItemCount(Other.MalachiteUrchinOnKill.itemDef) == 0)
                    {
                        ...
                        component2.inventory.GiveItem(Other.MalachiteUrchinOnKill.itemDef, character.inventory.GetItemCount(itemDef));
            ...
            {
                if (characterBody.master.teamIndex == teamIndex && characterBody.inventory.GetItemCount(Other.MalachiteUrchinOnKill.itemDef) > 0)
                {
                    if (NetworkServer.active)
                    {
                        for (int i = 0; i < characterBody.inventory.GetItemCount(Other.MalachiteUrchinOnKill.itemDef); i++)
                        {
                            ...
                            //characterBody.HandleOnKillEffectsServer(damageReport); idk if this is needed but it's commented out incase it appears it is needed
